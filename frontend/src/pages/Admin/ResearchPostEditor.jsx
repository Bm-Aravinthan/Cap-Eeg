...
const ResearchPostEditor = ({ isEdit }) => {
  ...
  //   if (!isDraft && !postData.tags.length) {
  //   setError("Please add some tags.");
  //   return;
  // }
    ...
    try {
      ...
      formData.append("generatedByAI", postData.generatedByAI || false);
      ...
      if (postData.coverImageUrl instanceof File) {
        formData.append("coverImage", postData.coverImageUrl);
      } else if (typeof postData.coverPreview === "string") {
        // include existing cover URL
        formData.append("coverImageUrl", postData.coverPreview);
      }
    ...
    // }
      ...
      Object.keys(images).forEach((key) => {
        ...
        if (img instanceof File) {
          ...
          // include existing image URLs for backend to keep them
    ...
    // });
      ...
      if (postData.document instanceof File) {
        formData.append("document", postData.document);
      } else if (typeof postData.document === "string") {
        // only keep it if it wasn't removed
        formData.append("documentUrl", postData.document);
      }
            ...
            <div className="mt-4">
              <CoverImageSelector
                ...
                setImage={(value) => {handleValueChange("coverImageUrl", value);
                  if (value === null) setIsCoverRemoved(true);}
                }
              ...
              {Object.keys(images).map((key) => {
                ...
                return (
                  <div key={key} className="relative group">
                    <label htmlFor={`researchImage${key}`} className="cursor-pointer">
                      <img
                        className="max-h-13 cursor-pointer opacity-90 w-28 h-28 object-cover rounded border border-gray-100 group-hover:opacity-70 transition"
                        src={previewSrc}
                        alt={`Research Image ${key}`}
                      />
                    </label>
                    ...
                    {(typeof img === "string" || img instanceof File) && (
                      <button
                        type="button"
                        className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 text-xs shadow-md opacity-0 group-hover:opacity-100 transition"
                        onClick={(e) => {
                          e.preventDefault();
                          // If existing Cloudinary image → mark it as removed
